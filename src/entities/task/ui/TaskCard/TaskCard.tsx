...
const TaskCard = ({ task, onDeleteTask, onUpdateTask }: TaskCardProps) => {
  ...
  if (isEditing) {
    return (
      <article
        ref={setNodeRef}
        style={style}
        className={`group relative rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition-all dark:border-slate-700 dark:bg-slate-800 ${
      ...
      >
        <div className="mb-5 flex items-center justify-between">
          ...
          <button
            ...
            {...attributes}
            {...listeners}
            className="cursor-grab rounded-lg px-2 py-1.5 text-slate-400 transition hover:bg-slate-100 hover:text-slate-700 active:cursor-grabbing dark:hover:bg-slate-700 dark:hover:text-slate-200"
        ...
        >
          <div className="flex flex-col gap-2">
            ...
            <input
              ...
              className={`rounded-xl border bg-white px-3 py-3 text-sm text-slate-800 outline-none transition-all dark:bg-slate-900 dark:text-slate-200 ${
                isTitleEmpty
          ...
          <div className="mt-2 flex gap-2 border-t border-slate-100 pt-4 dark:border-slate-700">
            <button
              type="submit"
              disabled={isTitleEmpty}
              className="rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-medium text-white transition hover:bg-blue-700 disabled:cursor-not-allowed disabled:bg-slate-400 dark:hover:bg-blue-500"
            >
              Save changes
            </button>
            ...
            <button
              ...
              className="rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm font-medium text-slate-700 transition hover:bg-slate-50 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-200 dark:hover:bg-slate-700"
            >
              Cancel
            </button>
          </div>
  ...
  return (
    <article
      ref={setNodeRef}
      style={style}
      className={`relative rounded-xl border border-slate-200 bg-white p-4 shadow-sm transition-all hover:shadow-md dark:border-slate-700 dark:bg-slate-800 ${
        isDragging ? 'border-blue-400 opacity-60 shadow-lg' : ''
      }`}
    >
      {isOver && !isDragging && (
